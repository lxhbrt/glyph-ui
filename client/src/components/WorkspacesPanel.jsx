...
/** 👁 nur lesen · 👁✎ lesen+schreiben · 🔒 gesperrt */
...
function modeLabel(m) {
  if (m === "rw") return "r+w";
  if (m === "private") return "gesperrt";
  return "r";
}

function modeTitle(m) {
  if (m === "rw")
    return "Lesen + Schreiben + Whitelist-Shell (elevated = Popup)";
  if (m === "private") return "Gesperrt (Schloss) — kein Zugriff für ^_Code";
  return "Nur Lesen (Auge) — kein Write, keine Shell";
}
...
export function WorkspacesPanel() {
  ...
  const [attachMode, setAttachMode] = useState("rw");
  ...
  const positions = useMemo(() => {
    ...
    workspaces.forEach((w, i) => {
      map[w.id] = nodePos(i, workspaces.length);
    });
    return map;
  }, [workspaces]);
  ...
  async function onAttach(e) {
    ...
    setError("");
    try {
      await api("POST", "/api/workspaces", { input: raw, mode: attachMode });
      setInput("");
      await refresh();
    } catch (err) {
  ...
  async function onDetach(id) {
    if (!id || busy) return;
    if (!window.confirm("Workspace vom Kabelsalat lösen?")) return;
    setBusy(true);
    setError("");
    try {
  ...
  async function patchWorkspace(id, patch) {
    ...
    setError("");
    try {
      await api("PATCH", `/api/workspaces/${encodeURIComponent(id)}`, patch);
      await refresh();
    } catch (err) {
      setError(err.message || String(err));
    } finally {
  ...
  function cycleMode(w) {
    const i = MODE_CYCLE.indexOf(w.mode || "r");
    const next = MODE_CYCLE[(i + 1) % MODE_CYCLE.length];
    void patchWorkspace(w.id, { mode: next });
  }

  return (
    <section className="vaults-panel workspaces-panel" aria-labelledby={titleId}>
      <h4 id={titleId} className="bindings-section-title">
        Kabelsalat · Workspaces
      </h4>
      ...
      <div className="vaults-map-stage" data-loading={loading || undefined}>
        ...
        >
          {workspaces.map((w) => {
            const to = positions[w.id];
            if (!to) return null;
            return (
              <line
                key={w.id}
                className={`vaults-cable ${modeClass(w.mode)}${
                  w.enabled === false ? " is-cut" : ""
                }${selectedId === w.id ? " is-selected" : ""}`}
                x1={50}
                y1={50}
                x2={to.x}
                y2={to.y}
        ...
        <div className="vaults-map-hub" title="^_Code">
          <span className="vaults-hub-label">^_Code</span>
        </div>

        {workspaces.map((w) => {
          const pos = positions[w.id] || { x: 50, y: 20 };
          return (
            <button
              key={w.id}
              type="button"
              className={`vaults-node ${modeClass(w.mode)}${
                w.primary ? " is-primary" : ""
              }${selectedId === w.id ? " is-selected" : ""}${
                !w.exists ? " is-missing" : ""
              }`}
            ...
            >
              {w.primary ? <span className="vaults-star">★</span> : null}
              <span className="vaults-node-name">{w.name}</span>
              <span className="vaults-node-mode">
                <ModeBadge mode={w.mode || "r"} />
              </span>
            </button>
          );
        })}
      ...
      <form className="vaults-attach" onSubmit={onAttach}>
        <input
          ...
          placeholder="Pfad · z. B. ~/glyph-ui oder /Users/…/projekt"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          disabled={busy}
          aria-label="Workspace anbinden"
        />
        <select
        ...
        >
          ...
          <option value="private">🔒 gesperrt</option>
        </select>
        ...
        <div className="vaults-detail">
          ...
          <div className="vaults-actions">
            ...
            <button
              type="button"
              className="ghost"
              onClick={() => void patchWorkspace(selected.id, { primary: true })}
            ...
            <button
              type="button"
              className="ghost"
              onClick={() => void patchWorkspace(selected.id, { move: "up" })}
              disabled={busy}
            >
              ↑
            </button>
            <button
              type="button"
              className="ghost"
              onClick={() => void patchWorkspace(selected.id, { move: "down" })}
              disabled={busy}
            >
              ↓
            </button>
      ...
      <ul className="vaults-list" aria-label="Workspace-Liste">
        {workspaces.map((w) => (
          <li key={w.id}>
            <button
              type="button"
              className={`vaults-list-row${selectedId === w.id ? " is-selected" : ""}`}
              onClick={() => setSelectedId(w.id)}
            >
              <span>
                {w.primary ? "★ " : ""}
              ...
              <span className={`vaults-list-mode ${modeClass(w.mode)}`}>
                <ModeBadge mode={w.mode || "r"} />
              </span>
            </button>
          </li>
